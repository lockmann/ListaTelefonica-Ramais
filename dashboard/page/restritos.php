<?php
	...
	if(isset($_POST['filtrar'])){
		$busca = $_POST['busca'];
		$sql = "SELECT * FROM restricted WHERE name LIKE '%$busca%' && status = 0 ORDER BY name ASC";
		$sql = mysqli_query($connect, $sql);
	}else{
		$sql = "SELECT * FROM restricted WHERE status = 0 ORDER BY name ASC";
		$sql = mysqli_query($connect, $sql);
	}
?>
				...
				<div class="card rounded border border-gray">
				    ...
				    <div id="collapse<?php echo $result['id'] ?>" class="collapse p-0 m-0" aria-labelledby="heading<?php echo $result['id'] ?>" data-parent="#accordion<?php echo $result['id'] ?>">
				      <div class="card-body p-0">
						<div class="container-fluid p-0 pt-2 pb-2 d-flex border border-bottom-1">
							<div class="pl-3 w-50 icon">
								<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-telephone-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
									  <path fill-rule="evenodd" d="M2.267.98a1.636 1.636 0 0 1 2.448.152l1.681 2.162c.309.396.418.913.296 1.4l-.513 2.053a.636.636 0 0 0 .167.604L8.65 9.654a.636.636 0 0 0 .604.167l2.052-.513a1.636 1.636 0 0 1 1.401.296l2.162 1.681c.777.604.849 1.753.153 2.448l-.97.97c-.693.693-1.73.998-2.697.658a17.47 17.47 0 0 1-6.571-4.144A17.47 17.47 0 0 1 .639 4.646c-.34-.967-.035-2.004.658-2.698l.97-.969z"/>
								</svg>
								<p class="d-inline mr-1">Fixo</p>
							</div>
							<div class="pl-3 w-50 icon">
								...
								<p class="d-inline mr-1">Celular</p>
							</div>
						</div>
						<div class="container-fluid p-0 d-flex">
							<div class="w-50">
								<table class="rounded table border table-sm m-0">
								 	<?php while ($result2 = $sql2->fetch_array()) { ?>
								 		<tr>
											<td class="pl-3">
												<?php 
													echo $result2['num'];
							...
							<div class="w-50">
								 <table class="rounded table border table-sm m-0">
								 	<?php while ($result2 = $sql3->fetch_array()) { ?>
								 		<tr>
											<td class="pl-3">
												<?php 
													echo $result2['num'] 
