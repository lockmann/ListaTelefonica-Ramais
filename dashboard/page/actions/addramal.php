<?php
    ...
    $busca = "SELECT branch_line.*, section. `name` AS section_id FROM `branch_line` INNER JOIN `section` ON `branch_line`.`section_id` = section.`id` WHERE num = '$numero' && status = 0 && type = 0";
    ...
    $busca_status = $busca['status'];
	    ...
	    if ($busca_num == $numero) {
	    	...
	    	$sql = "INSERT INTO branch_line (section_id, complement, num, status, private, type, insert_by, dt) VALUES ('$setor', '$complemento', '$numero', '0', '$private', '0', '$id', '$dt')";
...
<div class="container-fluid p-4 m-0">
	<div class="d-flex justify-content-between name-page">
		<h4 class="d-inline m-0">Adicionar Ramal</h4>
		<a href="?p=ramais">
